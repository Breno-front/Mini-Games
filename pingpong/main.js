class Ball {
    ...
    render() {
      ...
      if (this.images.length > 0) {
        ...
        if (this.isPlayer && !this.playerHasMoved) {
          textSize(14);
          fill(54);
          text("Use as Setas para se mover", this.x - padding, this.y - (this.height * 1.5));
        }
      } else {
        noStroke();
        fill(54);
  ...
  function playerCollisionEvent() {
    ...
    ball.x = paddleOne.x + paddleOne.width + 10;
    let velocityMag;
    ...
    if (ball.y < paddleOne.y) {
      ...
      ball.velocity.y = map(velocityMag, -6, 0, xVel * maxTopSlope, 0);
    } else {
      velocityMag = random(4);
      ball.velocity.y = map(velocityMag, 0, 6, 0, xVel * maxBottomSlope);
    }
  }
  
  function computerCollisionEvent() {
    ball.velocity.x *= -1.08;
    ball.x = paddleTwo.x - paddleTwo.width + 10;
    let velocityMag;
    let xVel = ball.velocity.x;
